...
foreach ($it in $appItems) {
  ...
  if ($isUwp) {
    ...
    if ($aumid) {
      $pfm = ($aumid -split '!')[0]
      $pkg = Get-AppxPackage | Where-Object { $_.PackageFamilyName -eq $pfm } | Select-Object -First 1
      if (-not $pkg) {
        $pkg = Get-AppxPackage -AllUsers -ErrorAction SilentlyContinue | Where-Object { $_.PackageFamilyName -eq $pfm } | Select-Object -First 1
      }

      if ($pkg) {
        try {
          $icons = Get-UwpAppIcons $pkg.Name
          $match = $icons | Where-Object { $_.AUMID -eq $aumid } | Select-Object -First 1
          if ($match -and $match.IconPath) { $iconPath = $match.IconPath }
        } catch { }
        $id = $pkg.Name
      } else { $id = $aumid }
      $location = "shell:AppsFolder\$aumid"
